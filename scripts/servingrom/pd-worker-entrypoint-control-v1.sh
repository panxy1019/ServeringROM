...
nohup python3 /opt/servingrom-control/pd_proxy.py --host 0.0.0.0 --port 8080 \
  ...
  --servingrom-control-enabled --servingrom-control-test-endpoints \
  --control-min-rho-a 0.2 --control-max-rho-a 0.8 \
  --control-max-step 0.2 --control-min-dwell-seconds 5 \
  --control-max-load-skew-tokens 2048 >"$LOG_DIR/proxy.log" 2>&1 &
