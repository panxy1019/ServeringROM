...
class PrefillOverloadedError(RuntimeError):
    """Raised when the token-aware prefill admission budget is exhausted."""


class RequestTokenEstimator:
    """Count the actual model tokens used by completion and chat requests."""

    def __init__(self, tokenizer_path: str):
        from transformers import AutoTokenizer

        self._tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, trust_remote_code=True)
        logger.info("Loaded proxy tokenizer from %s", tokenizer_path)

    def request_tokens(self, req_data: dict[str, Any]) -> int:
        ...
        if isinstance(prompt, list):
            # OpenAI completions permits token IDs as a prompt. Do not re-tokenize
            # them, because they already express the backend's work exactly.
            if prompt and isinstance(prompt[0], int):
                return len(prompt)
            prompt = "\n".join(str(item) for item in prompt)
        return len(self._tokenizer.encode(str(prompt), add_special_tokens=False))

    def text_tokens(self, text: str) -> int:
        return len(self._tokenizer.encode(text, add_special_tokens=False))


def get_token_estimator() -> RequestTokenEstimator:
    if token_estimator is None:
        raise RuntimeError("token estimator is not initialized")
    return token_estimator


def requested_output_tokens(req_data: dict[str, Any]) -> int:
    value = req_data.get("max_completion_tokens", req_data.get("max_tokens", 16))
    try:
...
class SharedProxyScheduler:
    ...
    def __init__(self, prefiller_instances, decoder_instances, *, max_prefill_inflight_tokens: int):
        ...
        self.prefill_waiting_tokens = 0
        self.prefill_admission_rejected_total = 0
        self.max_prefill_inflight_tokens = max_prefill_inflight_tokens
    ...
    def healthcheck(self) -> dict[str, Any]:
        with self._lock:
            return {
                ...
                "prefill_waiting_tokens": self.prefill_waiting_tokens,
                "max_prefill_inflight_tokens": self.max_prefill_inflight_tokens,
                "prefill_admission_rejected_total": self.prefill_admission_rejected_total,
    ...
    def _pick_fair_key(self, role: ServerRole) -> str:
        ...
        The upstream example embeds ordinal in the heap tuple, which makes all
        sequential zero-load requests choose the first Decode forever. Selection
        here is deliberately O(number_of_backends); this lab has two Decode
        replicas and correctness/fairness is more valuable than heap cleverness.
        """
        pool = self._pool(role)
        candidates = [
            (key, entry)
        ...
        if not candidates:
            raise RuntimeError(f"No available {role.value} servers")
        lowest = min(self._priority(role, entry, key) for key, entry in candidates)
        ties = sorted(
            ((key, entry) for key, entry in candidates if self._priority(role, entry, key) == lowest),
            key=lambda item: item[1].ordinal,
        )
        cursor = self._tie_cursor[role]
        self._tie_cursor[role] = cursor + 1
        return ties[cursor % len(ties)][0]
    ...
    def begin_request(self, load: float) -> dict[str, Any]:
        ...
        with self._lock:
            if (
                self.max_prefill_inflight_tokens > 0
                and self.prefill_inflight_tokens + load > self.max_prefill_inflight_tokens
            ):
                self.prefill_admission_rejected_total += 1
                raise PrefillOverloadedError(
                    "prefill token budget exhausted: "
                    f"inflight={self.prefill_inflight_tokens} request={int(load)} "
                    f"limit={self.max_prefill_inflight_tokens}"
                )
            picked = self._pick_server(ServerRole.PREFILL, load, kv_cache=True)
            self.prefill_inflight_tokens += int(load)
            self.request_num += 1
            return picked
    ...
    def release_prefill_kv(self, key: str, load: float) -> None:
        with self._lock:
            ...
            self.prefill_inflight_tokens = max(0, self.prefill_inflight_tokens - int(load))

    def release_decoder_tokens(self, key: str, load: float) -> None:
        with self._lock:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Local tokenizer path used for chat-template-aware request accounting.",
    )
    parser.add_argument(
        "--max-prefill-inflight-tokens",
        type=int,
        default=8192,
        help="Reject a new request with HTTP 429 when this prefill token budget would be exceeded.",
    )
...
async def assign_instances(
    ...
    *,
    ...
    runtime = get_runtime()
    args = get_global_args()
    # Scores are actual model work rather than JSON byte length. Prefill tracks
    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                    ...
                    async for chunk in stream_service_response_with_retry(
                        ...
                        if not first_token_logged and chunk:
                            ...
                            logger.info(
                                ...
                                instance_info.prefill_seconds * 1000,
                                (time.monotonic() - instance_info.assigned_at) * 1000,
                            )
                        ...
                        if isinstance(reported_completion_tokens, int):
                            ...
                            completion_tokens = get_token_estimator().text_tokens(generated_token)
                        newly_accounted = max(0, completion_tokens - accounted_decoder_tokens)
                        if newly_accounted:
                            await runtime.schedule(
                                "release_decoder_tokens", instance_info.decoder_key, newly_accounted
                            )
                            instance_info.decoder_score = max(0.0, instance_info.decoder_score - newly_accounted)
                            accounted_decoder_tokens += newly_accounted
                        if stop_reason == "recomputed":
                            ...
                            retry_prompt_tokens = get_token_estimator().request_tokens(req_data)
                            retry_output_tokens = requested_output_tokens(req_data)
                ...
                logger.info(
                    "PD_COMPLETE request_id=%s decode=%s:%s remaining_decode_tokens=%d",
                    instance_info.request_id,
                    instance_info.decoder_host,
                    instance_info.decoder_port,
                    int(instance_info.decoder_score),
                )
                released_kv = True
